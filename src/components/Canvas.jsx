...
export class Canvas extends Component {
    handleClick = () => {
        window.location.href = `${process.env.PUBLIC_URL}/sample.html`;
      }
    constructor(props) {
        super(props)
        this.width = 50
    ...
    displayPath(path) {
        ...
        carElement.className = "car"; // add CSS class for styling
        carElement.innerHTML = "&#128663;"; // add the car emoji
        ...
        for (let i = 1; i < path.length; i++) {
            setTimeout(() => {
                ...
                const edgeRect = document.getElementById(`edge-${minVertex}-${maxVertex}`).getBoundingClientRect();
                carElement.style.top = `${edgeRect.top + edgeRect.height / 2}px`;
                carElement.style.left = `${edgeRect.left + edgeRect.width / 2}px`;
            }, i*200);
        }
        setTimeout(() => {
            this._generateButton.disabled = false
            ...
            this._sizeSlider.disabled = false
            this.maze = null
            // carElement.remove(); // remove the car element when done
        }, path.length*200);
    }
    ...
    render() {
        ...
        return (
            <div>
                ...
                <div>
          <button onClick={this.handleClick} style={{ color: "white" }}>State Table</button>
          <Legend />
        </div>
                <div className="error">
